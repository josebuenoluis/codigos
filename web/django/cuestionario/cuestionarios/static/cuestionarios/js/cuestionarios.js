...
function consultaCiclosAsignaturas(e){
    ...
    let data = {"ciclo":"","asignatura":"","busqueda":"","filtros":""};
    data.filtros = "true";
    if(busqueda != ""){
    ...
    let endpoint = `/cuestionarios/?ciclo=${encodeURIComponent(data.ciclo)}&asignatura=${encodeURIComponent(data.asignatura)}&busqueda=${encodeURIComponent(data.busqueda)}&filtros=${encodeURIComponent(data.filtros)}`; 
    fetch(endpoint, {
        method: "GET",
        headers: {
